frappe.ui.form.on("Suit Reservation", {
                ...
                frappe.call({
                    ...
                    error: function(err) {
                        frappe.msgprint({
                            ...
                            message: __('<a href="/app/customer-measurement/new?customer=') + 
    ...
    deposit_amount(frm) {
        ...
        if (frm.doc.deposit_amount && frm.doc.total_estimated_rent && frm.doc.deposit_amount > frm.doc.total_estimated_rent) {
            frappe.msgprint({
...
frappe.ui.form.on("Reservation Item", {
    item_code(frm, cdt, cdn) {
        ...
        if (row.item_code) {
            const duplicate = frm.doc.reservation_items.some((item, idx) => 
                item.item_code === row.item_code && item.name !== row.name
            );
            if (duplicate) {
                frappe.msgprint({
                    title: __("Duplicate Item"),
                    message: __("Item Code ") + row.item_code + __(" is already added. Please select a different item."),
                    indicator: "red"
                });
    ...
    check_availability(frm, cdt, cdn) {
        ...
        if (!frm.doc.branch || !frm.doc.reservation_from || !frm.doc.reservation_to || !frm.doc.source_warehouse) {
            frappe.show_alert({
                message: __("Select Branch, Reserve From, Reserve To, and Source Warehouse first."),
                indicator: "orange"
            });
            return;
        }
...
function check_item_balance(frm, row) {
    ...
    frappe.call({
        ...
        callback: function(r) {
            if (!r || !r.message) {
                frappe.show_alert({ message: __("No response from availability API"), indicator: "red" });
                return;
            }
